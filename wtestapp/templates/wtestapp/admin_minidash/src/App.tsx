...
const App: React.FC = () => {
  ...
  const [filterStatus, setFilterStatus] = useState<
    "all" | "completed" | "pending" | "in-progress" | "not-started"
  >("all");
  ...
  const percentWidth = (value: number, total: number) =>
  ...
  const fetchResearchers = async () => {
    try {
      ...
      const apiUrl = qs ? `/api/researchers/?${qs}` : "/api/researchers/";
      ...
      if (data.success) {
        ...
        console.log("Researchers set:", data.researchers.length, "doctors");
      } else {
  ...
  const fetchSurveys = async () => {
    try {
      ...
      if (filterMode !== "all") params.append("mode", filterMode);
      const qs = params.toString();
      const apiUrl = qs ? `/api/surveys/?${qs}` : "/api/surveys/";
      const response = await fetch(apiUrl);
      const data = await response.json();
      if (data.success) {
  ...
  // Frontend filtering only for search term and status (mode and survey are already filtered by API)
  const filteredResearchers = researchers.filter((researcher) => {
    ...
    const matchesStatus =
      filterStatus === "all" || researcher.status === filterStatus;
    
    const result = matchesSearch && matchesStatus;
    return result;
  });

  console.log("Filter state:", { filterMode, filterStatus, selectedSurveyId });
  console.log("Researchers from API:", researchers.length);
  console.log("Filtered researchers:", filteredResearchers.length);

  // Use filtered researchers for accurate counts based on current filters
  const totalResearchers = researchers.length;
  const completedSurveys = researchers.filter(
    (r) => r.status === "completed"
  ).length;
  const pendingSurveys = researchers.filter(
    (r) => r.status === "pending"
  ).length;
  const inProgressSurveys = researchers.filter(
    (r) => r.status === "in-progress"
  ).length;
  const notStartedSurveys = researchers.filter(
    (r) => r.status === "not-started"
  ).length;
  const cpResearchers = researchers.filter((r) => r.mode === "CP").length;
  const gcResearchers = researchers.filter((r) => r.mode === "GC").length;
  const completionRate =
    totalResearchers > 0
      ? Math.round((completedSurveys / totalResearchers) * 100)
      : 0;
  ...
  const exportToExcel = async () => {
    try {
      ...
      const qs = params.toString();
      const url = qs
        ? `/api/export-doctors-excel/?${qs}`
        : "/api/export-doctors-excel/";
      ...
      const a = document.createElement("a");
      a.href = dlUrl;
      a.download = "doctors_survey_export.xlsx";
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-pink-100 text-lg">
              Real-time survey monitoring and researcher management
            </p>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="flex items-center mt-2">
                ...
                <span className="text-green-500 text-sm font-medium">
                  {completedSurveys} of {totalResearchers}
                </span>
              </div>
            </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 mt-1">
                {pendingSurveys + inProgressSurveys}
              </p>
              <div className="flex items-center mt-2">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <span className="text-gray-500 text-sm ml-1">
                  ({percent(cpResearchers, totalResearchers)}%)
                </span>
              </div>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-3">
              <div
                ...
                style={{ width: percentWidth(cpResearchers, totalResearchers) }}
              ></div>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <span className="text-gray-500 text-sm ml-1">
                  ({percent(gcResearchers, totalResearchers)}%)
                </span>
              </div>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-3">
              <div
                ...
                style={{ width: percentWidth(gcResearchers, totalResearchers) }}
              ></div>
            </div>
          </div>
        </div>
      ...
      {!isLoading && (
        <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
          <div className="flex flex-col gap-4 mb-6">
            ...
            {(filterMode !== "all" || selectedSurveyId !== "all") && (
              <div className="flex flex-wrap items-center gap-3 px-4 py-3 bg-gradient-to-r from-pink-50 to-teal-50 rounded-lg border border-pink-200">
                ...
                <div className="flex items-center gap-2">
                  <span className="px-3 py-1 bg-pink-100 text-pink-800 rounded-full text-xs font-medium">
                    CP:{" "}
                    {filteredResearchers.filter((r) => r.mode === "CP").length}
                  </span>
                  <span className="px-3 py-1 bg-teal-100 text-teal-800 rounded-full text-xs font-medium">
                    GC:{" "}
                    {filteredResearchers.filter((r) => r.mode === "GC").length}
                  </span>
                ...
                {selectedSurveyId !== "all" && (
                  <span className="text-xs text-gray-600 ml-2">
                    Survey:{" "}
                    {surveys.find((s) => String(s.id) === selectedSurveyId)
            ...
            <div className="flex flex-col lg:flex-row gap-3">
              ...
              <select
                className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent min-w-[140px]"
                value={filterMode}
              ...
              <select
                className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent min-w-[180px]"
              ...
              >
                ...
                {surveys.map((s) => (
                  <option key={s.id} value={String(s.id)}>
                    {s.title} ({s.assigned_count} assigned)
                  </option>
                ))}
              </select>
              <select
                className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent min-w-[160px]"
                value={filterStatus}
              ...
              <button
                ...
                className="flex items-center justify-center px-4 py-2 bg-gradient-to-r from-teal-600 to-pink-600 text-white rounded-lg hover:from-teal-700 hover:to-pink-700 transition-all duration-200 whitespace-nowrap"
              >
              ...
              <button
                onClick={exportToCSV}
                className="flex items-center justify-center px-4 py-2 bg-gradient-to-r from-pink-600 to-teal-600 text-white rounded-lg hover:from-pink-700 hover:to-teal-700 transition-all duration-200 whitespace-nowrap"
              >
                <Download className="w-4 h-4 mr-2" />
                Export CSV
              </button>
              ...
              <div className="text-center py-12">
                ...
                {(selectedSurveyId !== "all" ||
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-gradient-to-r from-pink-600 to-teal-600 text-white rounded-lg hover:from-pink-700 hover:to-teal-700 transition-all duration-200"
                  >
                    Clear All Filters
                  </button>
                )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
          ...
          <div className="text-sm text-green-600 font-medium">
            {completedSurveys} of {totalResearchers} completed
          </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
          ...
          <div className="text-3xl font-bold text-gray-900 mb-2">
            {researchers.length > 0
              ? Math.round(
                  researchers.reduce((sum, r) => sum + r.surveyProgress, 0) /
                    researchers.length
                )
              : 0}
            %
      ...
      <div className="bg-gradient-to-r from-pink-600 via-teal-500 to-cyan-400 rounded-2xl p-8 text-white">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-pink-100 text-lg">
              Comprehensive researcher profiles and management •{" "}
          ...
          >
            ...
            <div className="flex items-center justify-between">
              <span className="text-xs text-gray-500">
                Last active: {researcher.lastActivity}
              </span>
              <div className="flex items-center space-x-2">
      ...
      <div className="bg-gradient-to-r from-pink-600 via-teal-500 to-cyan-400 rounded-2xl p-8 text-white">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-pink-100 text-lg">
              Real-time survey completion monitoring • {completedSurveys}/
              {totalResearchers} Completed
            </p>
          </div>
          <button
            onClick={handleRefresh}
              ...
              .map((researcher, index) => (
                <div key={researcher.id} className="flex items-center">
                  ...
                  <div className="flex-grow">
                    ...
                    <div className="text-sm text-gray-600">
                      {researcher.unit} • {researcher.specialty}
                    </div>
                    <div className="text-xs text-gray-500">
                      Completed on {researcher.completionDate}
                    </div>
                  </div>
                  <div className="text-right">
      ...
      <div className="bg-gradient-to-r from-pink-600 via-teal-500 to-cyan-400 rounded-2xl p-8 text-white">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-pink-100 text-lg">
              Manage survey timelines • {inProgressSurveys} Active •{" "}
            ...
            {[
              {
                ...
                task: "Survey Completion",
                priority: "high",
              },
              {
                date: "Jan 27",
                researcher: "Dr. Thomas Anderson",
                task: "Progress Review",
                priority: "medium",
              },
              {
                date: "Jan 30",
                researcher: "Dr. Amanda Taylor",
                task: "Initial Survey",
                priority: "low",
              },
              {
                date: "Feb 02",
                researcher: "Dr. Christopher Davis",
                task: "Follow-up",
                priority: "medium",
              },
            ].map((deadline, index) => (
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="text-sm text-gray-500">
                    {researcher.status === "completed"
                      ? `Completed ${researcher.completionDate}`
                      : researcher.status === "in-progress"
                      ? "In Progress"
  ...
  const renderSettings = () => (
    <div className="space-y-8">
      <div className="bg-gradient-to-r from-pink-600 via-teal-500 to-cyan-400 rounded-2xl p-8 text-white">
        ...
        <p className="text-pink-100 text-lg">
          Manage system preferences and data export options
        </p>
      </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-sm text-gray-600">
                  Receive alerts for survey completions
                </div>
              </div>
              <div className="relative">
                <input type="checkbox" className="sr-only" defaultChecked />
          ...
          <div className="space-y-4">
            <div className="p-4 border border-gray-200 rounded-lg">
              ...
              <div className="text-sm text-gray-600 mb-3">
                Export all researcher data and survey responses
              </div>
            ...
            <div className="p-4 border border-gray-200 rounded-lg">
              ...
              <div className="text-sm text-gray-600 mb-3">
                Select specific fields and date ranges
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
          ...
          <p className="text-gray-600 mb-4">
            Comprehensive guides and tutorials for using the dashboard
          </p>
            ...
            {[
              {
                ...
                answer:
                  "Click the 'Add Researcher' button in the Researchers section and fill out the required information including name, contact details, and assignment.",
              },
              {
                question: "How can I track survey completion progress?",
                answer:
                  "Navigate to the Survey Status page where you can see real-time progress for each researcher, including completion percentages and status updates.",
              },
              {
                question: "How do I export survey data?",
                answer:
                  "You can export data by clicking the 'Export CSV' button in the main dashboard or going to Settings {'>'}  Data Export.",
              },
            ].map((faq, index) => (
        ...
        <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
          ...
          <div className="space-y-4">
            <div className="p-4 bg-gray-50 rounded-lg">
              ...
              <a
                href="mailto:connect@wmefi.co.in"
  ...
  const renderCurrentPage = () => {
    switch (currentPage) {
      ...
      case "researchers":
        return renderResearchers();
      case "survey-status":
        return renderSurveyStatus();
      case "schedule":
  ...
  return (
    ...
    >
      ...
      <div
        ...
        } shadow-xl transition-all duration-300 flex flex-col border-r`}
      >
        <div
